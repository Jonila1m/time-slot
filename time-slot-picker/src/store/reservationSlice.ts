...
const reservationSlice = createSlice ({
    ...
    reducers: {
        setReservation: (state, action: PayloadAction<SelectedSlot>) => {
            const reservationIndex = state.selectedSlots.findIndex(slot => slot.companyId === action.payload.companyId)
        ...
        removeReservation: (state, action: PayloadAction<number>) => {
            state.selectedSlots = state.selectedSlots.filter((slot) => slot.companyId !== action.payload)
        }
    },
})
